        ...
        match request {
            ...
            AppRequest::KeytermAdd {
                ...
                if name.trim().is_empty() {
                    return Err(WireError::from_service(ServiceError::InvalidArgument(
                        "keyterm name must not be empty".to_string(),
                    )));
                }
            ...
            AppRequest::KeytermList => self
                ...
                .list_keyterms()
                .await
                .map(AppResponse::Keyterms)
                .map_err(Self::db_error),
            AppRequest::KeytermGet { id } => {
                let id = self
                    .db
                    .resolve_keyterm_id(&id)
                    .await
                    .map_err(Self::db_error)?;
                self.db
                    .find_keyterm(&id)
            ...
            AppRequest::KeytermModify {
                ...
                if name.is_none() && description.is_none() && content.is_none() {
                    return Err(WireError::from_service(ServiceError::NothingToModify));
                }
                let id = self
                    .db
                    .resolve_keyterm_id(&id)
            ...
            AppRequest::KeytermDelete { id } => {
                let id = self
                    .db
                    .resolve_keyterm_id(&id)
                    .await
                    .map_err(Self::db_error)?;
                self.db.delete_keyterm(&id).await.map_err(Self::db_error)?;
                Ok(AppResponse::Id { id })
            }
